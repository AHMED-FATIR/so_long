...
/*   Updated: 2023/02/10 00:51:23 by afatir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	f_wall(char *file, t_data *data, int *ln, int *len)
{
	...
	data->g_y = ft_count(file);
	*len = data->g_y;
	data->map = ft_calloc((data->g_y + 1), sizeof(char *));
	if (!data->map)
		return (0);
	fd = open(file, O_RDONLY);
	if (fd < 0)
		return (ft_free(data, 3), 0);
	data->map[0] = get_next_line(fd);
	if (!data->map[0] || !first_w(data->map[0]))
		return (ft_free(data, 3), 0);
...
int	ft_valid(char *file, t_data *data)
{
	...
	v.i = 1;
	if (!f_wall(file, data, &v.ln, &v.len))
		return (0);
	...
	while (v.i < v.len)
	{
		...
		if (!data->map[v.i])
			break ;
		if (!side_w(data->map, v.i, (v.ln - 1)))
			return (ft_free(data, 4), 0);
		v.i++;
	}
	if (!last_w(data->map[v.i - 1]))
		return (ft_free(data, 5), 0);
	close(v.fd);
	return (1);
}
